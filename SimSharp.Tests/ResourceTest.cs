...
namespace SimSharp.Tests
{
    ...
    public class ResourceTest
    {
        ...
        [TestMethod]
        public void TestResourceWithQuantity()
        {
            var start = new DateTime(2014, 4, 1);
            var env = new Environment(start);
            var resource = new Resource(env, capacity: 6);

            var log = new Dictionary<string, DateTime>();
            env.Process(TestResourceWithQuantity(env, "a", resource, log));
            env.Process(TestResourceWithQuantity(env, "b", resource, log));
            env.Run();
        ...
        private IEnumerable<Event> TestResourceWithQuantity(Environment env, string name, Resource resource,
          Dictionary<string, DateTime> log)
        {
            using (Request req = resource.Request(4))
            {
                yield return req;
                Assert.AreEqual(4, resource.InUse);
                Assert.AreEqual(2, resource.Remaining);
                yield return env.Timeout(TimeSpan.FromSeconds(1));
            }
